...
export const achievements = pgTable("achievements", {
  ...
  category: text("category").notNull(), // e.g., "reports", "training", "participation", "accuracy"
  iconName: text("icon_name").notNull(), // Lucide icon name
  iconColor: text("icon_color").default("#6366f1"), // Hex color for the icon
  requirements: jsonb("requirements").notNull(), // Criteria for earning this achievement
  points: integer("points").notNull().default(10), // Points awarded for this achievement
...
export const userAchievements = pgTable("user_achievements", {
  ...
  earnedAt: timestamp("earned_at").defaultNow(),
  progress: jsonb("progress"), // Additional progress data if needed
});
...
export const userGameProfile = pgTable("user_game_profile", {
  ...
  streak: integer("streak").default(0), // Current streak of consecutive days with activity
  longestStreak: integer("longest_streak").default(0),
  lastActiveDate: date("last_active_date"),
  badges: jsonb("badges").default([]), // Array of badge IDs or names
  preferences: jsonb("preferences").default({}), // Notification preferences, etc.
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const leaderboards = pgTable("leaderboards", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  type: text("type").notNull(), // "weekly", "monthly", "all_time", "event_based"
  category: text("category").notNull(), // "points", "reports", "accuracy", "participation"
  startDate: timestamp("start_date"),
  endDate: timestamp("end_date"),
  isActive: boolean("is_active").default(true),
  rewards: jsonb("rewards"), // Rewards for top performers
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const leaderboardEntries = pgTable("leaderboard_entries", {
  ...
  rank: integer("rank"),
  metadata: jsonb("metadata"), // Additional data like report count, accuracy percentage, etc.
...
// Define insert schemas for achievement system
...
export const insertUserGameProfileSchema = createInsertSchema(userGameProfile)
  .omit({
    id: true,
    createdAt: true,
    updatedAt: true,
