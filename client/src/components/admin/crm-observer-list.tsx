...
const CRMObserverList: React.FC = () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['crm-observers'] }),
  });
          ...
          {filteredData && filteredData.length > 0 ? (
            filteredData.map((observer: any) => (
              <tr key={observer.id} className="border-t">
                <td className="p-2">{observer.CRMContact?.name || 'N/A'}</td>
                ...
                <td className="p-2 flex gap-2">
                  ...
                  <button className="text-red-600 hover:underline" onClick={() => deleteMutation.mutate(observer.id)} disabled={deleteMutation.isLoading}>
            ...
            <tr><td colSpan={6} className="p-2 text-gray-500">No observers found.</td></tr>
          )}
        </tbody>
      </table>
      ...
      {linkModalOpen && linkObserver && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow-lg p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold mb-4">Link Observer to User Profile</h2>
            <div className="mb-4">
              <input
                ...
                {userResults.length > 0 ? (
                  userResults.map((user: any) => (
                    <li key={user.id} className="flex justify-between items-center py-1 border-b">
                      ...
                      <button
                        className="bg-primary text-white px-2 py-1 rounded text-xs ml-2"
